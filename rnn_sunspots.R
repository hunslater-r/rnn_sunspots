...
#Adatbehívás:
data <- sunspots #Beépített R-adatokkal dolgozunk
head(data)

#Néhány általános tudnivalü:
is.ts(data)
#[1] TRUE
...
#Data frame-mé alakítunk:
data_df <- as.data.frame(data)
names(data_df) <- c("qty")

###Idoadatok - évszázad (c), évtized (d), év (y), hónap (month):
library(zoo)
...
#Training Ã©s test setek szÃétválasztása:
train <- as.data.frame(data_df[as.numeric(as.character(data_df$year)) < 1960,]) #Training: 1960 elott
...
##################################
## Modellezés az RNN csomaggal: ##
##################################

library(rnn) #RNN csomag behívása
set.seed(12345)
...
library(plyr)
train_min <- apply(data.matrix(train), 2, FUN = min) #Training-adatok minimumainak kiszámítása
train_max <- apply(data.matrix(train), 2, FUN = max) #Training-adatok maximumainak kiszámítása
...
# Lépés 2.: Data matrix (numerikus mátrix)
...
#Valós és elorejelzett adatok összevetése ábrán:
